...
class WikiProcessor(DataProcessor):
    ...
    def _create_examples(self, data_dir, split, label=None):
        ...
        :return:
        """
        df = pd.read_csv(os.path.join(data_dir, f"wiki_{split}.csv"))
        examples = list()
        for row in df.itertuples():
            example = InputExample(text_a=row.comment, guid=row.rev_id)
            example.label = int(row.is_toxic)
